...
-- NOTE: This file is an executable DDL draft. Do not remove UNIQUE/CHECK/FK constraints
--       that enforce idempotency or owner-consistency invariants.
...
-- -----------------------------------------------------------------------------
-- public_id: UUID v7 generated by the application; stored as uuid type.
...
-- 1.5 market_state (runtime execution state; composite PK)
CREATE TABLE IF NOT EXISTS market_state (
  owner_id             bigint NOT NULL,
  market               varchar(32) NOT NULL,
  trade_status         varchar(16) NOT NULL CHECK (trade_status IN ('ACTIVE','SUSPENDED')),
  suspend_reason_code  varchar(64),
  ...
  updated_at           timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (owner_id, market),
  CONSTRAINT fk_market_state_market_config
    FOREIGN KEY (owner_id, market)
    REFERENCES market_config(owner_id, market)
    ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS ix_market_state_owner_trade_status ON market_state(owner_id, trade_status);

-- 1.6 kill_switch (shared PK)
...
CREATE TABLE IF NOT EXISTS signal (
  ...
  CONSTRAINT ux_signal_id_owner UNIQUE (id, owner_id)
);

CREATE INDEX IF NOT EXISTS ix_signal_owner_created_at ON signal(owner_id, created_at);
CREATE INDEX IF NOT EXISTS ix_signal_owner_market_close_time ON signal(owner_id, market, candle_close_time DESC);
...
CREATE TABLE IF NOT EXISTS order_intent (
  ...
  CONSTRAINT ux_order_intent_id_owner UNIQUE (id, owner_id),
  CONSTRAINT fk_order_intent_signal_owner
    FOREIGN KEY (signal_id, owner_id)
    REFERENCES signal(id, owner_id)
    ON DELETE RESTRICT,
  CONSTRAINT ck_order_intent_requested_amount CHECK (
    (side = 'BUY'  AND requested_krw IS NOT NULL AND requested_krw > 0 AND requested_volume IS NULL)
    OR
    (side = 'SELL' AND requested_volume IS NOT NULL AND requested_volume > 0 AND requested_krw IS NULL)
  )
);

CREATE INDEX IF NOT EXISTS ix_order_intent_owner_created_at ON order_intent(owner_id, created_at);
CREATE INDEX IF NOT EXISTS ix_order_intent_owner_status ON order_intent(owner_id, status);
...
CREATE TABLE IF NOT EXISTS order_attempt (
  ...
  CONSTRAINT fk_order_attempt_intent_owner
    FOREIGN KEY (order_intent_id, owner_id)
    REFERENCES order_intent(id, owner_id)
    ON DELETE CASCADE,
  CONSTRAINT ck_order_attempt_acked_has_uuid CHECK (status <> 'ACKED' OR upbit_uuid IS NOT NULL),
  CONSTRAINT ck_order_attempt_throttled_has_retry CHECK (status <> 'THROTTLED' OR next_retry_at IS NOT NULL)
);

CREATE UNIQUE INDEX IF NOT EXISTS ux_order_attempt_upbit_uuid
  ON order_attempt(upbit_uuid)
...
CREATE TABLE IF NOT EXISTS upbit_order (
  ...
  executed_volume numeric(38,18) NOT NULL DEFAULT 0 CHECK (executed_volume >= 0),
  created_at      timestamptz NOT NULL DEFAULT now(),
  updated_at      timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT ux_upbit_order_uuid_owner UNIQUE (upbit_uuid, owner_id),
  CONSTRAINT ck_upbit_order_price_non_negative CHECK (price IS NULL OR price >= 0),
  CONSTRAINT ck_upbit_order_volume_non_negative CHECK (volume IS NULL OR volume >= 0)
);
...
CREATE TABLE IF NOT EXISTS fill (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT fk_fill_upbit_uuid_owner
    FOREIGN KEY (upbit_uuid, owner_id)
    REFERENCES upbit_order(upbit_uuid, owner_id)
    ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS position (
  ...
  avg_price  numeric(38,18) NOT NULL DEFAULT 0 CHECK (avg_price >= 0),
  status     varchar(16) NOT NULL CHECK (status IN ('FLAT','OPEN')),
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (owner_id, market)
);
