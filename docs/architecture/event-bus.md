...
**스테일 락 복구**: 별도 sweeper 없이 claim 대상에 만료된 `PROCESSING`을 포함한다(1안 채택).  
claim 대상 조건: `status = 'PENDING' AND next_retry_at <= now()` **OR** `status = 'PROCESSING' AND locked_until < now()`.
...
### 6.3 스테일 락 복구(고정 — 1안)
워커가 죽으면 `PROCESSING` 상태가 남는다. 복구는 **1안(claim 쿼리 직접 reclaim)** 으로 고정한다.

- 별도 sweeper 없이 **claim 쿼리가 만료된 PROCESSING을 직접 reclaim**한다.
- claim 대상 조건: `status = 'PENDING' AND next_retry_at <= now()` **OR** `status = 'PROCESSING' AND locked_until < now()` (§6.2 SQL과 동일).
