...
| **request_json** | **jsonb** | **Upbit CreateOrder payload canonical snapshot(실행 시점 고정)** |
...
역할: order_intent는 semantic intent, order_attempt.request_json은 executable snapshot. outbox CreateOrderAttempt payload는 orderAttemptId만 전달.
...
제약(필수):
- PRIMARY KEY(market, timeframe, candle_time) 또는 UNIQUE(market, timeframe, candle_time)
