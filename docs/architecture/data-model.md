...
| id | bigint PK(identity) | |
| public_id | uuid unique | 외부 노출 UUID v7 |
| owner_id | bigint FK(app_user.id) | Signal.owner_id와 동일해야 함 |
...
| id | bigint PK(identity) | |
| public_id | uuid unique | 외부 노출 UUID v7 |
| owner_id | bigint FK(app_user.id) | OrderIntent.owner_id와 동일해야 함 |
...
|---|---|---|
| id | bigint PK(identity) | |
| owner_id | bigint FK(app_user.id) | UpbitOrder.owner_id와 동일해야 함 |
