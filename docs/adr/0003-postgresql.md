...
- 스키마 변경은 **Hibernate ddl-auto**로 관리한다(v2 MVP: `spring.jpa.hibernate.ddl-auto: update`). 엔티티 기준으로 테이블 생성·컬럼 추가가 적용된다.
...
- 스키마는 엔티티에서 생성·갱신되므로, 복잡한 수동 마이그레이션(Flyway 등) 없이 개발 단계를 단순화할 수 있다. 운영에서 스키마 변경 이력이 필요해지면 ddl-auto를 `validate`로 바꾸고 마이그레이션 도구를 도입할 수 있다.
