...
public class TradingService {
    ...
    /**
     * 사용자와 마켓에 대한 트레이딩 시그널을 처리합니다.
     * 
     * @param signal 트레이딩 시그널
     * @param user 사용자
     * @return 처리 성공 여부
     */
    @Transactional
    public boolean processSignal(TradingSignal signal, User user) {
        ...
        boolean buyProcessed = false;
        boolean sellProcessed = false;
        
        // 매수 주문 로직
        if (buySignal && botSetting.getIsBuyActive()) {
            buyProcessed = processBuyOrder(signal, user, marketCode, botSetting, buyStrategy, buySignalStrength);
        }
        
        // 매도 주문 로직
        if (sellSignal && botSetting.getIsSellActive()) {
            sellProcessed = processSellOrder(signal, user, marketCode, botSetting, sellStrategy, sellSignalStrength);
        }
        
        return buyProcessed || sellProcessed;
    }
    
    private boolean processBuyOrder(TradingSignal signal, User user, String marketCode, BotSetting botSetting, 
                                   Strategy buyStrategy, double buySignalStrength) {
    ...
    private boolean processSellOrder(TradingSignal signal, User user, String marketCode, BotSetting botSetting, 
                                    Strategy sellStrategy, double sellSignalStrength) {
